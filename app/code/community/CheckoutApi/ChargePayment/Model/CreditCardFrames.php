...
class CheckoutApi_ChargePayment_Model_CreditCardFrames extends CheckoutApi_ChargePayment_Model_Checkout
{
    ...
    public function authorize(Varien_Object $payment, $amount) {
        ...
        if(strpos($requestData['customer_card'], 'checkoutapiframes') !== false){
            ...
            if($result->isValid()) {
                ...
                if (!empty($errors['errorCode'])) {
                    ...
                    $errorMessage = Mage::helper('chargepayment')->__('Authorize action is not available1.');
                }
        ...
        if (empty($cardToken) && empty($checkoutApiCardId)) {
            Mage::throwException(Mage::helper('chargepayment')->__('Authorize action is not available2.'));
        ...
        if($result->isValid()) {
            ...
            if ($isDebug) {
                ...
                if (!empty($errors['errorCode'])) {
                    ...
                    $errorMessage = Mage::helper('chargepayment')->__('Authorize action is not available3.');
                }
            } else {
                $errorMessage = Mage::helper('chargepayment')->__('Authorize action is not available4.');
            }
    ...
    private function _getCharge($amount = null, $quoteId = null) {
        ...
        $billAddressData = $billAddress->getData();
        $shipAddress = $checkout->getShippingAddress();
        $shipAddressData = $shipAddress->getData();

        // $street = Mage::helper('customer/address')
            // ->convertStreetLines($shippingAddress->getStreet(), 2);

        $billingAddressConfig = array (
            'addressLine1'  => $billAddressData['street'],
            'addressLine2'  => $billAddressData['street'],
            'postcode'      => $billAddressData['postcode'],
            'country'       => $billAddressData['country_id'],
            'city'          => $billAddressData['city'],
            'state'         => $billAddressData['region'],
            'phone'         => array('number' => $billAddressData['telephone'])
        );
        ...
        $shippingAddressConfig = array(
            'recipientName'      => $shipAddressData['firstname'] .' '. $shipAddressData['lastname'],
            'addressLine1'       => $shipAddressData['street'],
            'addressLine2'       => $shipAddressData['street'],
            'postcode'           => $shipAddressData['postcode'],
            'country'            => $shipAddressData['country_id'],
            'city'               => $shipAddressData['city'],
            'state'              => $shipAddressData['region'],
            'phone'         => array('number' => $shipAddressData['telephone'])
        );
